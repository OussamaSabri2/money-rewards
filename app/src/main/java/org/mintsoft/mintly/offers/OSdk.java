...
public class OSdk extends Fragment {
    ...
    public void onViewCreated(@NonNull View view, Bundle savedInstanceState) {
        ...
        sAdapter adapter = new sAdapter(context);
        recyclerView.setAdapter(adapter);
    }

    private class sAdapter extends RecyclerView.Adapter<sAdapter.ViewHolder> {
        ...
        public sAdapter.ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
            ...
            return new sAdapter.ViewHolder(view);
        }

        @Override
        public void onBindViewHolder(@NonNull sAdapter.ViewHolder holder, int position) {
            HashMap<String, String> data = Offers.hashList.get("offer_sdk").get(position);
        ...
        public int getItemCount() {
            return Offers.hashList.get("offer_sdk").size();
        }
            ...
            public void onClick(View view) {
                try {
                    ...
                    Class<?> c = Class.forName(packagename + ".sdkoffers."
                            + Offers.hashList.get("offer_sdk").get(pos).get("name"));
                    startActivity(new Intent(context, c).putExtra("user", userId)
                            .putExtra("info", Offers.hashList.get("offer_sdk").get(pos)));
